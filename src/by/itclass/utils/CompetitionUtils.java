...
import static by.itclass.constans.CompetitionConstants.*;

@UtilityClass // static поля и методы , приватный конструктор
public class CompetitionUtils {
    ...
    private static void processException(Map<String, String> errors, CompetitionException exception) {
        ...
        if (matcher.find()) {
            errors.put(matcher.group(), String.format("Error in line \"%s\" - %s", exception.getErrorLine(), exception.getCause().getMessage()));
        }
    }
